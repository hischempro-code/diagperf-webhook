...
const { VALID_INTENTS, VALID_STATES } = require("./intent-router");
...
// ====== Sortie structurée (output_config.format) ======
// Garantit un JSON valide conforme à l'union {intent|answer|route}, supprimant à la RACINE
// le bug "Haiku répond en texte brut au lieu du JSON" (cf. [[llm-plaintext-not-json]]).
// Schéma PLAT (racine=objet, seul `type` requis) car un `anyOf` racine n'est pas garanti,
// et l'API plafonne à 24 paramètres OPTIONNELS (nested compris). `data` volontairement
// minimal : le webhook downgrade les routes profondes vers WAITING_PLATE et re-lit
// plaque/véhicule, donc seuls plate/vehicleYear/skip/reason sont réellement exploités.
// Enums tirés d'intent-router (source unique) pour rester en phase. Aucun beta header requis.
const LLM_RESPONSE_SCHEMA = {
  type: "object",
  properties: {
    type: { type: "string", enum: [...VALID_LLM_TYPES] },
    intent: { type: "string", enum: [...VALID_INTENTS] },
    message: { type: "string" },
    sendLocation: { type: "boolean" },
    target: { type: "string", enum: [...VALID_STATES] },
...
// Kill-switch env (défaut ON) : LLM_STRUCTURED_OUTPUTS=false désactive sans redéploiement.
// Le filet de parsing (texte brut, JSON tronqué) reste actif dans tous les cas — au cas où
// une réponse tronquée par max_tokens ne respecte pas le schéma (stop_reason max_tokens).
const USE_STRUCTURED_OUTPUTS = String(process.env.LLM_STRUCTURED_OUTPUTS || "true").toLowerCase() !== "false";
...
- ⛔ Ne CITE JAMAIS de chiffre de gain précis (ex: "+30 ch", "+45 Nm", "+26 %") de mémoire : les gains dépendent du moteur exact et sont calculés par le SYSTÈME (fiche véhicule après lecture de plaque). Parle en termes génériques ("gains de puissance et de couple personnalisés selon votre motorisation").
- ⛔ Ne CITE JAMAIS l'année, la cylindrée, le code moteur, la puissance ou toute caractéristique technique d'un véhicule que le client ne t'a pas communiquée. Ces données viennent UNIQUEMENT de la lecture de plaque par le système. En cas de doute, demande la plaque.
...
async function askLLM(userMessage, waId) {
  ...
  try {
      ...
      try {
        ...
        if (shouldSummarize(memory, total) && _summarizeActive < MAX_CONCURRENT_SUMMARIES) {
          ...
          _getRecentMessages(waId, 30)
            // getRecentMessages renvoie DÉJÀ { role, content } (le format attendu par
            // summarizeAndStore). L'ancien .map() re-dérivait role/content depuis
            // m.direction/m.body — des champs INEXISTANTS sur cet objet → tous les messages
            // devenaient { role:"assistant", content:undefined } → résumé "Assistant: undefined"
            // ×30 stocké en mémoire puis réinjecté dans le prompt système. On passe msgs tel quel.
    ...
    // Corps construit une fois (pas par tentative). La sortie structurée est un ajout : le
    // schéma est CONSTANT donc le prompt cache reste valide (il n'est invalidé que si le
    // format CHANGE entre requêtes).
    const requestBody = {
      model: LLM_MODEL,
      system: systemBlocks,
    ...
    if (USE_STRUCTURED_OUTPUTS) {
      requestBody.output_config = { format: { type: "json_schema", schema: LLM_RESPONSE_SCHEMA } };
    }
    let resp;
    ...
    // ⚠️ NE PAS supprimer les filets ci-dessous en pensant que les sorties structurées
    // (output_config) les rendent morts. Ils restent PORTEURS pour deux raisons :
    //  1. kill-switch LLM_STRUCTURED_OUTPUTS=false → on repasse en JSON "best effort" (Haiku
    //     répond alors souvent en texte brut / avec backticks — cf. mémoire llm-plaintext) ;
    //  2. même schéma imposé, stop_reason=max_tokens TRONQUE le JSON en pleine génération
    //     → le salvage "JSON tronqué" plus bas récupère l'intent/le message partiel.
    ...
    if (!VALID_LLM_TYPES.has(parsed.type)) {
      // Réutilise VALID_INTENTS importé d'intent-router (source de vérité du schéma) —
      // plus de copie locale en dur qui divergerait si un intent était ajouté.
    ...
    // Télémétrie anti-hallucination MOTORISATION : le LLM affirme le carburant d'un
    // véhicule spécifique ("votre X est un diesel/essence"). Sans confirmation plaque
    // c'est probablement inventé (cf. prod 15/07 "la C1 est diesel" + FAP/EGR sur essence).
    // Non bloquant : on mesure les récidives ; la prévention se fait par prompt + flow plaque.
    if (parsed.type === "answer" &&
        /votre\s+[^.!?]{0,30}?\b(?:est|es|équipée?|dot[ée]e?|roule\s+(?:au|en)|de\s+type)\b[^.!?]{0,35}?\b(?:diesel|essence)\b/i.test(parsed.message || "")) {
      _log.warn("Hallucination-risque: motorisation affirmée sur véhicule spécifique", { wa_id: waId, snippet: (parsed.message || "").slice(0, 140) });
    }
