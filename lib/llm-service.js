...
Cas 3 — Vraiment incompréhensible ET sans contexte exploitable dans l'historique (premier message bizarre, chiffre isolé sans contexte, plaque envoyée sans sujet...) :
{ "type": "menu" }
⚠️ Utilise Cas 3 UNIQUEMENT si même l'historique ne permet pas de comprendre ce que veut le client.
...
💡 Devis gratuit et sans engagement.`;
    ...
    if (waId) {
      try {
        ...
        // Déclencher un résumé async si la conversation est longue
        const total = memory.profile?.total_user_messages || 0;
        if (shouldSummarize(memory, total)) {
