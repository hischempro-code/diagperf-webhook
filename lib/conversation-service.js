...
async function getRecentMessages(waId, limit = 6) {
  try {
    ...
    // ts est un epoch en SECONDES : deux messages dans la même seconde (in Meta / out serveur)
    // s'ordonnaient de façon arbitraire → question/réponse pouvaient s'inverser dans l'historique.
    // Tie-breaker sur id (auto-incrément) pour un ordre déterministe.
    const { data: msgs, error } = await _supabase
      .from("messages")
      .select("id, direction, body, ts")
