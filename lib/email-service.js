...
async function _send({ to, subject, text, html, pdfBuffer, pdfName }) {
  ...
  if (pdfBuffer) {
    body.attachment = [{ content: pdfBuffer.toString("base64"), name: pdfName || "devis.pdf" }];
  }
...
async function sendRdvClientEmail({ to, firstName, lastName, vehicleDesc, prestationLabel, devisRef, htTxt, ttcTxt, contactReason, pdfBuffer }) {
  if (!to || !process.env.BREVO_API_KEY) return;
  ...
  try {
    await _send({
      ...
      html:
        ...
        `</div>`,
      pdfBuffer,
      pdfName: `${devisRef}.pdf`,
    });
    _log.info("sendRdvClientEmail: envoi OK", { to, devisRef, hasAttachment: !!pdfBuffer });
    return true;
  } catch (err) {
