...
function buildVehiclePerformanceCard(vehicle) {
  ...
  const isEssence = fuelType === "essence" || fuelType === "unknown";
  const isDiesel = fuelType === "diesel";
  const isHybridOrEV = fuelType === "hybrid" || fuelType === "electric";

  const gains = power ? estimateGains(power, fuelType, vehicle.engine_cc) : null;
  ...
  if (isHybridOrEV) {
    ...
    if (isEssence) {
      lines.push(`   ✅ E85 — ${e85PriceText(year)} • ⏱ 1h`);
      lines.push(`   ❌ FAP / EGR / AdBlue _(moteur essence)_`);
    } else if (isDiesel) {
      const fapPrice = (year && year >= 2019) ? FAP_FIXED_PRICE_NEW : FAP_FIXED_PRICE_OLD;
      lines.push(`   ✅ Suppression FAP — *${fapPrice}€ TTC* • ⏱ 3-4h`);
      lines.push(`   ✅ Suppression EGR / AdBlue — sur devis`);
      lines.push(`   ❌ E85 _(moteur diesel)_`);
    }

    lines.push("");
    lines.push(`🛡️ *Garantie 2 ans* incluse — Devis gratuit, sans engagement`);
    lines.push("");
  }
