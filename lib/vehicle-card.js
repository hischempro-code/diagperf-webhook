 ...
 * Returns null si le moteur est trop faible, atmosphérique, ou si le turbo est INCERTAIN
 * — dans ce cas la fiche affiche "gains personnalisés" au lieu d'un chiffre inventé.
 * @param {string} rawEngineText - fuel+trim bruts (détection marqueurs turbo : TSI, TCe…)
 */
function estimateGains(powerHp, fuelType, engineCc, rawEngineText = "") {
  if (!powerHp || powerHp < 70 || powerHp > 500) return null;
  const cc = engineCc || 1600;
  const hpPerLiter = powerHp / (cc / 1000);

  // Turbo CERTAIN uniquement — sinon pas de chiffre (anti sur-promesse).
  // - Diesel : quasi tous turbo depuis 2000 (HDi/TDI/dCi…) → seuil bas suffit.
  // - Essence : l'ancien seuil (>55 ch/L OU >130ch) classait turbo une Clio 1.6 atmo
  //   110ch (69 ch/L) ou un V6 atmo → promettait +26% là où un atmo gagne ~8-10%.
  //   Exiger ≥85 ch/L (turbo moderne) OU un marqueur turbo explicite dans fuel/trim.
  let isTurbo;
  if (fuelType === "diesel") {
    isTurbo = hpPerLiter > 45 || powerHp > 110;
  } else {
    const turboMarker = /\b(tsi|tfsi|tce|thp|puretech|ecoboost|turbo|biturbo|t-?jet|dig-?t|tgi)\b/i.test(String(rawEngineText || ""));
    isTurbo = hpPerLiter >= 85 || turboMarker;
  }
...
/**
 * Économies E85 annuelles estimées — modulées par la cylindrée et présentées comme
 * BORNE HAUTE ("jusqu'à"). L'ancienne valeur fixe "~700 €/an" sur-promettait pour les
 * petites cylindrées (ex: C1 998cc qui consomme peu). Les paliers restent des estimations
 * hautes (kilométrage moyen), honnêtes car cadrées "jusqu'à".
 */
function e85SavingsText(engineCc) {
  const cc = parseInt(engineCc, 10) || 0;
  let maxSaving;
  if (!cc) maxSaving = 700;          // cylindrée inconnue → estimation générique
  else if (cc < 1200) maxSaving = 400;  // citadines (C1, Twingo…)
  else if (cc < 2000) maxSaving = 700;  // compactes / berlines
  else maxSaving = 1000;                // grosses cylindrées
  return `jusqu'à *~${maxSaving} €/an*`;
}
...
function buildVehiclePerformanceCard(vehicle) {
  ...
  const gains = (power && !isHybridOrEV && !isUnknownFuel)
    ? estimateGains(power, fuelType, vehicle.engine_cc, `${fuel} ${vehicle.trim || ""}`)
    : null;
  ...
  if (isHybridOrEV) {
    ...
    // ── Économies E85 ──
    if (isEssence) {
      lines.push(`⛽ *E85 Flexfuel :* économies estimées ${e85SavingsText(vehicle.engine_cc)}`);
    }
