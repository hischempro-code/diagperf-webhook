...
// Requires capitalized name words to avoid false matches on regular sentences.
function extractContactFromText(text) {
  ...
  const nameWords = words.filter(w =>
    !w.includes("@") &&
    /^[A-ZÀÂÄÉÈÊËÎÏÔÖÙÛÜÇŒÆ]/u.test(w) &&
    /^[a-zA-ZÀ-ÿ\-']+$/u.test(w) &&
    w.length > 1
  );
  return {
    email: emailWord.toLowerCase(),
    name: nameWords.length >= 1 ? nameWords.slice(0, 3).join(" ") : null,
  };
}
