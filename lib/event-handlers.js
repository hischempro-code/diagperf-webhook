...
// ⚠️ Meta refuse le texte libre hors fenêtre de service 24h : le garage n'écrit jamais
// au bot, donc CHAQUE envoi échouait en "(#100) Invalid parameter" → une ligne ERROR
// par devis dans les logs + bruit Sentry. La notification garage passe par EMAIL (Brevo),
// qui fonctionne. Pour réactiver WhatsApp (nécessite un template Meta approuvé) :
// poser GARAGE_WA_NOTIFY=true sur Render.
async function notifyGarage(message) {
  const garageWaId = process.env.GARAGE_WA_ID;
  const enabled = (process.env.GARAGE_WA_NOTIFY || "false").toLowerCase() === "true";
  if (!garageWaId || !enabled) {
    _log.debug("notifyGarage: désactivé (GARAGE_WA_NOTIFY≠true) — notification garage par email uniquement", { preview: message.slice(0, 60) });
    return;
  }
  try {
    ...
    _log.warn("notifyGarage: échec envoi (best effort, l'email garage reste la source fiable)", { to: garageWaId, error: String(err?.message || err) });
  }
}
...
async function handleFrustrationEscalation(fromWa, text, rawMsg) {
  ...
  // Urgence TECHNIQUE (panne, voyant, "ne démarre plus") SANS colère : c'est un lead
  // diagnostic chaud → demander au client s'il veut un diagnostic AVANT de basculer en
  // rappel humain. On ne passe en escalade humaine directe que si le client est en colère.
  const technicalUrgency = signal.critical && signal.level !== "angry";
  if (technicalUrgency) {
    _log.info("Urgence technique → proposition diagnostic (au lieu d'escalade directe)", {
      wa_id: fromWa, score: signal.score, reasons: signal.reasons, preview: text.slice(0, 120),
    });
    try {
      await _sendWhatsAppInteractiveButtons(
        fromWa,
        `🔧 J'ai bien noté un souci technique sur votre véhicule.\n\n` +
        `Souhaitez-vous que je lance un *diagnostic* pour identifier le problème, ou préférez-vous être rappelé(e) par un conseiller ?`,
        [
      ...
      _log.error("Escalation technique: erreur envoi", { wa_id: fromWa, error: String(err?.message || err) });
    }
    notifyGarage(
      `🔧 SOUCI TECHNIQUE CLIENT (diagnostic proposé)\n` +
      `WhatsApp : ${fromWa}\n` +
