...
async function fetchVehicleFromPlate(plate) {
  ...
  // Timeout réseau : un blocage TCP (ex: IP de sortie non autorisée côté API) fait
  // sinon traîner la requête ~18s et menace le timeout du webhook Meta.
  // On loggue aussi err.cause.code pour identifier la panne (ETIMEDOUT / ENOTFOUND / cert...).
  let resp;
  try {
    ...
    _log.error("Immatriculation API: échec réseau (fetch failed)", {
      plaque: normalized,
      message: String(netErr?.message || netErr),
      code: cause?.code || netErr?.code || (netErr?.name === "AbortError" ? "TIMEOUT_12S" : null),
      cause: cause ? String(cause.message || cause) : null,
    });
    throw new Error("IMMATRICULATION_API_NETWORK");
  }
