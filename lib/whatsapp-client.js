...
async function _callWhatsAppAPI(to, payload, { endpoint = "messages", logLabel = "WhatsApp", retries = 2, retryDelayMs = 600 } = {}) {
    ...
    try {
      ...
      if (!resp.ok) {
        ...
        if (isRetryable && attempt < retries) {
          ...
          _log.warn(`${logLabel} API error, retry ${attempt + 1}/${retries}`, { status: resp.status, to, wait });
      ...
      // Réseau flaky : retente uniquement les erreurs réseau (pas les erreurs API 4xx)
      if (err?.message?.includes("failed:")) throw err;
      const wait = retryDelayMs * Math.pow(2, attempt);
      _log.warn(`${logLabel} network error, retry ${attempt + 1}/${retries}`, { to, error: String(err?.message || err), wait });
      await new Promise(r => setTimeout(r, wait));
    }
  }
}
...
// Format officiel Meta : l'indicateur "en train d'écrire" se demande via le message_id
// du message ENTRANT et marque aussi le message comme lu (coches bleues).
// L'ancien payload ({to, typing_action}) n'existe pas dans l'API → échouait en silence.
async function sendTypingIndicator(messageId) {
  try {
    ...
    _log.debug("sendTypingIndicator failed", { messageId, error: String(err?.message || err) });
  }
}
