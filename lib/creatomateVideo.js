...
async function renderStageGainsVideo({ vehicle, stage, stageLabel, priceTtc }) {
  ...
  // If an element doesn't exist in the template, Creatomate silently ignores it — so we send
  // both the compact MVP placeholders (vehicle_name, stage_label) AND the rich ones for future.
  const stageLabelUpper = (stageLabel || "STAGE 1").toUpperCase();
  const stageSummary = `${stageLabelUpper}\n[size 150%]+${hpGain}ch / +${torqueGain}Nm[/size]`;

  const modifications = {
    // === MVP Quick Promo template (only 2 text slots) ===
    "vehicle_name.text": vehicleName || "VOTRE VÉHICULE",
    "stage_label.text": stageSummary,
    // === Extended placeholders (ignored if not in template) ===
