...
async function renderStageGainsVideo({ vehicle, stage, stageLabel, priceTtc }) {
  ...
  const modifications = {
    "vehicle_name": vehicleName || "VOTRE VÉHICULE",
    "vehicle_engine": engineTxt || "",
    "stage_label": (stageLabel || "STAGE 1").toUpperCase(),
    "hp_before": String(hpBefore),
    "hp_after": String(hpAfter),
    "hp_gain": `+${hpGain}`,
    "torque_before": String(torqueBefore),
    "torque_after": String(torqueAfter),
    "torque_gain": `+${torqueGain}`,
    "zero_to_hundred": zeroToHundredTxt,
    "price_ttc": priceTtc || "",
  };
