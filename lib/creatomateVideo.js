...
async function renderStageGainsVideo({ vehicle, stage, stageLabel, priceTtc }) {
  ...
  // Non-existent elements are silently ignored by Creatomate.
  const stageLabelUpper = (stageLabel || "STAGE 1").toUpperCase();

  const modifications = {
    "vehicle_name.text": vehicleName || "VOTRE VÉHICULE",
    "vehicle_engine.text": engineTxt || "",
    "stage_label.text": stageLabelUpper,
    "hp_before.text": String(hpBefore),
    "hp_after.text": String(hpAfter),
    "hp_gain.text": `+${hpGain} CHEVAUX`,
    "torque_before.text": String(torqueBefore),
    "torque_after.text": String(torqueAfter),
    "torque_gain.text": `+${torqueGain} NM`,
    "zero_to_hundred.text": zeroToHundredTxt ? `0-100 KM/H ${zeroToHundredTxt}` : "",
...
/**
 * Renders a prestation premium video for E85/FAP/ADBlue.
 * Uses dedicated template IDs per intent (env vars CREATOMATE_TEMPLATE_ID_E85, _FAP, _ADBLUE).
 *
 * @param {Object} args
 * @param {"E85"|"FAP"|"ADBLUE"} args.intent
 * @param {Object} args.vehicle  - { make, model, engine_cc, fuel, power_hp }
 * @param {string} args.priceTtc - ex: "490€ TTC"
 * @returns {Promise<string|null>}
 */
async function renderPrestationVideo({ intent, vehicle, priceTtc }) {
  const apiKey = process.env.CREATOMATE_API_KEY;
  ...
  const templateId = templateEnvMap[intent];

  if (!apiKey || !templateId) {
    log.warn(`Creatomate ${intent}: template not configured (missing CREATOMATE_TEMPLATE_ID_${intent})`);
    return null;
  }
  if (!vehicle) {
    log.warn("renderPrestationVideo: missing vehicle");
    return null;
  }
  ...
  const engineTxt = [
    vehicle.engine_cc ? `${(vehicle.engine_cc / 1000).toFixed(1)}L` : null,
    vehicle.fuel,
    vehicle.power_hp ? `${vehicle.power_hp}ch` : null,
  ].filter(Boolean).join(" ");

  // Only vehicle-specific fields are dynamic — metric values are baked into each template
  const modifications = {
    "vehicle_name.text": vehicleName || "VOTRE VÉHICULE",
    "vehicle_engine.text": engineTxt || "",
    "price_ttc.text": priceTtc || "",
  ...
  try {
    log.info(`Submitting ${intent} render`, { vehicleName, priceTtc });
    ...
    if (!resp.ok) {
      const errTxt = await resp.text().catch(() => "");
      log.error(`${intent} API error`, { status: resp.status, body: errTxt.slice(0, 300) });
      return null;
    }

    const json = await resp.json();
    const render = Array.isArray(json) ? json[0] : json;
    if (!render) { log.error(`${intent} empty response`); return null; }

    if (render.status === "succeeded" && render.url) {
      log.info(`${intent} render ready immediately`, { id: render.id });
      return render.url;
    }
    ...
    log.warn(`${intent} render did not complete in time`, { status: render.status });
    return null;
  } catch (err) {
    log.error(`${intent} unexpected error`, { error: String(err?.message || err) });
    return null;
  }
}

module.exports = { renderStageGainsVideo, renderPrestationVideo };
