...
async function generateQuotePdf({ devisRef, date, vehicleDesc, plate, prestationLabel, stageLabel, gainTxt, htTxt, ttcTxt, tvaTxt, customerName, customerEmail, customerPhone }) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const textX = hasLogo ? ML + 130 : ML;
    if (hasLogo) {
      try { doc.image(logoPath, ML, 10, { height: 115 }); } catch {}
    }

    doc.fontSize(24).font("Helvetica-Bold").fillColor(C.white).text("DIAGPERF", textX, 22);
    doc.fontSize(9).font("Helvetica").fillColor(C.faint).text("Reprogrammation & Diagnostic automobile", textX, 54);
    doc.fontSize(8).fillColor("#475569").text("38 Rue Jean Pierre Plicque, 77124 Villenoy", textX, 70);
    doc.fontSize(8).fillColor("#475569").text("contact@diagperf.com   |   06 75 54 70 85", textX, 83);
