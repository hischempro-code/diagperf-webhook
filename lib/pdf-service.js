...
async function generateQuotePdf({ devisRef, date, vehicleDesc, plate, prestationLabel, stageLabel, gainTxt, htTxt, ttcTxt, tvaTxt, customerName, customerEmail, customerPhone }) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const textX = hasLogo ? ML + 128 : ML;
    if (hasLogo) {
      try {
        // Logo 500×500 : voiture à pixels y:125-260, x:100-400
        // On veut afficher y:8-118 en PDF = pixels 125-260
        // Scale = 110/(260-125)*500 ≈ 407px total
        // y_img = 8 - 125*(407/500) = -94
        // x_img centré sur voiture (pixel x:250 → PDF x:103) = -100
        doc.save();
        doc.rect(ML, 8, 118, 110).clip();
        doc.image(logoPath, -100, -94, { height: 407 });
        doc.restore();
      } catch {}
    }
