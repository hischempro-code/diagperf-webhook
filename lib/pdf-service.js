...
async function generateQuotePdf({ devisRef, date, vehicleDesc, plate, prestationLabel, stageLabel, gainTxt, htTxt, ttcTxt, tvaTxt, customerName, customerEmail, customerPhone }) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // ── HEADER BAND ──────────────────────────────────────────────────────────
    doc.rect(0, 0, PW, 108).fill(C.headerBg);
    doc.rect(0, 108, PW, 4).fill(C.accent);

    // Logo
    const logoPath = path.join(__dirname, "..", "assets", "logo.png");
    const hasLogo = fs.existsSync(logoPath);
    const textX = hasLogo ? ML + 90 : ML;
    if (hasLogo) {
      try { doc.image(logoPath, ML, 12, { height: 84 }); } catch {}
    }

    doc.fontSize(24).font("Helvetica-Bold").fillColor(C.white).text("DIAGPERF", textX, 20);
    doc.fontSize(9).font("Helvetica").fillColor(C.faint).text("Reprogrammation & Diagnostic automobile", textX, 50);
    doc.fontSize(8).fillColor("#475569").text("38 Rue Jean Pierre Plicque, 77124 Villenoy", textX, 65);
    doc.fontSize(8).fillColor("#475569").text("contact@diagperf.com   |   06 75 54 70 85", textX, 78);

    // Reference card (top right inside header)
    const refW = 150, refH = 84;
    const refX = MR - refW;
    ...
    doc.fontSize(8).font("Helvetica").fillColor("#93c5fd")
      .text("N° DEVIS", refX + 12, 22, { width: refW - 16, align: "center" });
    doc.fontSize(18).font("Helvetica-Bold").fillColor(C.white)
      .text(devisRef, refX + 12, 36, { width: refW - 16, align: "center" });
    doc.moveTo(refX + 14, 62).lineTo(refX + refW - 6, 62).strokeColor(C.accent).lineWidth(0.5).stroke();
    doc.fontSize(8).font("Helvetica").fillColor("#93c5fd")
      .text(date, refX + 12, 68, { width: refW - 16, align: "center" });
