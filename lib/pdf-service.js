...
async function generateQuotePdf({ devisRef, date, vehicleDesc, plate, prestationLabel, stageLabel, gainTxt, duration, htTxt, ttcTxt, tvaTxt, customerName, customerEmail, customerPhone }) {
  ...
  return new Promise((resolve, reject) => {
    ...
    // Logo : clip path pour masquer le padding blanc du PNG 500×500
    // L'image est scalée grande (280px) et décalée pour que la voiture
    // tombe dans la zone visible du header.
    const logoPath = path.join(__dirname, "..", "assets", "logo.png");
    const hasLogo = fs.existsSync(logoPath);
    const textX = hasLogo ? ML + 138 : ML;
    if (hasLogo) {
      try {
        // Voiture dans le PNG 500×500 : x:55-430 (375px), y:80-310 (230px)
        // Scale pour tenir en 130px de large : 130/375 = 0.347 → image=173px
        // x_img : centre clip (44+65=109) - centre car (242.5*0.347=84) = 25
        // y_img : 5 - 80*0.347 = -23 → car top à PDF y≈5, bottom à y≈85
        doc.save();
        doc.rect(ML, 5, 130, 120).clip();
        doc.image(logoPath, 25, -23, { height: 173 });
        doc.restore();
      } catch {}
    }
    ...
    section(C.green, C.greenBg, C.green, "PRESTATION", prestH, (x, cy) => {
      ...
      doc.fontSize(9).font("Helvetica").fillColor(C.muted)
        .text(`Durée d'intervention : ${duration || "variable selon la prestation"}`, x, cy);
      if (gainTxt) {
        cy += 20;
...
async function sendQuotePdf(fromWa, { devisId, plate, vehicle, prestationLabel, stageLabel, gainTxt, duration, devisRow, customerName, customerEmail, customerPhone }) {
  try {
    ...
    const pdfBuffer = await generateQuotePdf({
      ...
      prestationLabel: prestationLabel || "N/A", stageLabel, gainTxt, duration,
      htTxt, ttcTxt, tvaTxt,
