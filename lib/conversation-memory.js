...
function buildMemoryContext({ profile, summary }) {
  ...
  if (hasProfileInfo) {
    ...
    if (profile.preferred_tone === "tu") {
      lines.push(`• Ton préféré : TUTOIEMENT (le client tutoie, fais de même)`);
    } else if (profile.preferred_tone === "vous") {
      lines.push(`• Ton préféré : VOUVOIEMENT (le client vouvoie, fais de même)`);
    }
