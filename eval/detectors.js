...
const RE_MOTORISATION = /votre\s+[^.!?]{0,30}?\b(?:est|es|équipée?|dot[ée]e?|roule\s+(?:au|en)|de\s+type)\b[^.!?]{0,35}?\b(?:diesel|essence)\b/i;
...
//    apparaît aussi dans un refus correct ("nous n'intervenons PAS à domicile").
const RE_DOMICILE = /d[ée]placement\s+(?:à\s+)?domicile|d[ée]placement\s+possible|[àa]\s+votre\s+domicile|intervention\s+group[ée]e|partenariat\s+local|malgr[ée]\s+la\s+distance|on\s+peut\s+s['e]\s*arranger|garage\s+proche\s+de\s+chez\s+vous/i;
...
const DETECTORS = {
  motorisation_affirmee: {
    ...
    detect: (text) => _match(RE_MOTORISATION, text),
  },
  faux_devis: {
  ...
  deplacement_domicile: {
    label: "Déplacement/intervention à domicile évoqué (interdit — atelier uniquement)",
    detect: (text) => _match(RE_DOMICILE, text),
  },
  garantie_chiffree: {
